              ...
              <div className="flex items-end gap-2">
                <div className="relative">
                  ...
                  {templatesOpen && <div className="absolute bottom-11 left-0 z-10 w-80 rounded-lg border border-slate-200 bg-white p-2 shadow-xl">
                    ...
                    {templatesLoading ? <p className="p-3 text-xs text-slate-400">Loading templates...</p> : templates.length === 0 ? <p className="p-3 text-xs text-slate-500">No approved templates are synced yet. Select “Sync from Meta”.</p> : templates.map((template) => <button key={template.id} type="button" onClick={() => { setSelectedTemplate(template); setTemplatesOpen(false); setComposer(''); }} className="block w-full rounded-md px-3 py-2 text-left hover:bg-slate-50"><span className="block text-xs font-bold text-slate-800">{template.name}</span><span className="text-[10px] text-slate-500">{template.category} · {template.language}</span></button>)}
                  </div>}
                </div>
